...
Status BufHashTbl::insert(const File* file, const int pageNo, const int frameNo) {
  ...
  hashBucket* tmpBuc = ht[index];
    ...
    // Debug log
    std::cout << "Inserting page " << pageNo << " of file " << file
    << " into frame " << frameNo << " at index " << index << std::endl;
    
  while (tmpBuc) {
      if (tmpBuc->file == file && tmpBuc->pageNo == pageNo){
          std::cout << "Duplicate entry found for page " << pageNo << " of file " << file << std::endl;
          return HASHTBLERROR;
      }
    tmpBuc = tmpBuc->next;
  }
  ...
  tmpBuc = new hashBucket;
    if (!tmpBuc){
        std::cout << "tmpBuc is NULL" << std::endl;
        return HASHTBLERROR;
    }
